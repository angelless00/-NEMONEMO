...
class MainView(QMainWindow, form_class):
    ...
    def showlist(self):
        ...
        text = [
            "24-11-14_18:22:13_103",
            "24-11-14_18:24:13_103",
            "24-11-14_18:30:13_103",
            "24-11-14_18:40:13_103",
            "24-11-14_18:40:13_103",
            "24-11-14_18:40:13_103",
            "24-11-14_18:40:13_103",
            "24-11-14_18:40:13_103",
    ...
    def load_video(self):
        ...
        if self.video_file:
            ...
            self.video_thread.pixmap_signal.connect(
                self.update_frame
            )  # 영상 cv정보(pixmap)를 받아오는 시그널, QPix... 쟤는 메인 쓰레드에서 처리되어야한다.
            self.video_thread.video_playing_signal.connect(
                self.playing_label_change
            )  # VideoOfThread에서 length (int) 받아옴
            self.video_thread.info_signal.connect(
                self.video_totaltime_label
            )  # VideoOfThread에서 length (int), fps (float) 받아옴
            self.video_thread.info_signal.connect(self.print_video_info)
            self.video_thread.silder_signal.connect(
                self.silder_change
            )  # VideoOfThread에서 length (int), fps (float), current_time (int) 받아옴
    ...
    def video_FileLoad(self):
        fname = QFileDialog.getOpenFileName(
            self
        )  # QFileDialog 자체가 파일 불러오기 기능. App2 디렉토리가 열린다.
...
class ThreadOfVideo(QThread, form_class):
    ...
    def run(self):
        if self.video_file:
            ...
            )  # info_signal을 통해 메인쓰레드로 변수 전송
            while self.running:
                self.ret, self.frame = (
                    cap.read()
                )  # 알아서 0번부터 끝까지 착실히 넘어가면서 수행
                ...
                if self.move_control == "after":
                    ...
                    self.ret = cap.set(
                        cv2.CAP_PROP_POS_FRAMES, self.now_frame + self.fps * 10
                    )
                ...
                if self.ret:
                    ...
                    self.convertToQtFormat = QImage(
                        self.rgbImage.data,
                        self.rgbImage.shape[1],
                        self.rgbImage.shape[0],
                        QImage.Format_RGB888,
                    )
                    self.pixmap = QPixmap(
                        self.convertToQtFormat
                    )  # 이 부분이 메인스레드에서 처리되어야하는 QPixmap이 있어서 2개 다 잡아먹고 있는거다?
                    self.p = self.pixmap.scaled(
                        2050, 1150, QtCore.Qt.IgnoreAspectRatio
                    )  # 프레임 크기 조정 1920, 1080 후보 1
                    # frame_num = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
                    self.current_time = (
                        cap.get(cv2.CAP_PROP_POS_MSEC) * 0.001
                    )  # 밀리초 단위 현재 위치
                    self.video_playing_signal.emit(
                        self.current_time
                    )  # video_playing_signal을 통해 메인쓰레드로 변수 전송
                    ...
                    sleep(
                        0.01
                    )  # 영상 1프레임당 0.01초로 이걸로 영상 재생속도 조절하면됨 0.02로하면 0.5배속인거임
